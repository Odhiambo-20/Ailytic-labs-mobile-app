...
class _DronesPageState extends State<DronesPage> {
  ...
  Future<void> _fetchDrones() async {
    ...
    try {
      final rows = await BackendApi.instance.fetchDrones();
      final mapped = rows.map(_toDroneData).toList(growable: false);

      if (!mounted) return;
      setState(() {
        droneApplications = mapped;
        loading = false;
      });
  ...
  DroneData _toDroneData(Map<String, dynamic> raw) {
    final specs = raw['specifications'];
    String features = '';
    if (specs is List) {
      features = specs.map((e) => e.toString()).where((e) => e.trim().isNotEmpty).take(3).join(' | ');
    } else if (specs is Map) {
      features = specs.entries.map((e) => '${e.key}: ${e.value}').take(3).join(' | ');
    } else if (specs is String) {
      features = specs;
    }

    final type = (raw['type'] ?? 'General').toString();
    final price = (raw['price'] ?? '\$0').toString();

    return DroneData(
      id: int.tryParse((raw['id'] ?? 0).toString()) ?? 0,
      name: (raw['name'] ?? 'Unnamed Drone').toString(),
      application: type,
      type: type,
      description: (raw['description'] ?? 'No description available').toString(),
      image: (raw['image'] ?? '').toString(),
      features: features.isEmpty ? 'Enterprise-grade drone system' : features,
      price: price.isEmpty ? '\$0' : price,
      rating: double.tryParse((raw['rating'] ?? 0).toString()) ?? 0,
      reviews: int.tryParse((raw['reviews'] ?? 0).toString()) ?? 0,
    );
  }
...
class _OrderPageState extends State<OrderPage> {
  ...
  Future<void> _handleSubmit() async {
    ...
    if (product == null) {
      setState(() {
        _isProcessing = false;
        _paymentError = 'Order item not found.';
      });
      return;
    }
    ...
    try {
      ...
      if (userId == null || userId.isEmpty) {
        throw const ApiException('Please login first to make a payment');
      }

      final totals = _calculateTotals();
      final total = double.tryParse(totals['total'] ?? '0') ?? 0;
      final idempotencyKey = '${DateTime.now().millisecondsSinceEpoch}-${product.id}-${paymentMethod.toUpperCase()}';

      Map<String, dynamic> response;
      if (paymentMethod == 'stripe') {
        response = await BackendApi.instance.createStripePayment({
          'userId': userId,
          'amount': total,
          'currency': 'USD',
          ...
          'metadata': {
            ...
            'cardHolder': cardName,
            'cardLast4': _extractLast4(cardNumber),
            'expiryDate': expiryDate,
          },
        });
      } else {
        final mpesaPhone = _formatMpesaPhone(phone);
        response = await BackendApi.instance.createMpesaPayment({
          'userId': userId,
          'amount': total,
          'currency': 'KES',
          'paymentMethod': 'MPESA',
      ...
      setState(() {
        ...
        _paymentId = (response['paymentId'] ?? response['checkoutRequestId'] ?? response['transactionId'] ?? 'PAY-${DateTime.now().millisecondsSinceEpoch}').toString();
      });
    } on ApiException catch (e) {
      if (!mounted) return;
      setState(() {
  ...
  String _extractLast4(String value) {
    final digits = value.replaceAll(RegExp(r'\D'), '');
    if (digits.length <= 4) return digits;
    return digits.substring(digits.length - 4);
  }

  String _formatMpesaPhone(String value) {
    final digitsOnly = value.replaceAll(RegExp(r'\D'), '');
    if (digitsOnly.isEmpty) {
      throw const ApiException('Phone number is required for M-Pesa payment');
    }
    ...
    if (phoneNumber.length != 12) {
      throw const ApiException('Invalid phone number. Enter a valid Kenyan mobile number, e.g. 0712345678');
    }
...
class _RobotsPageState extends State<RobotsPage> {
  ...
  Future<void> _fetchRobots() async {
    ...
    try {
      final rows = await BackendApi.instance.fetchRobots();
      final mapped = rows.map(_toRobotItem).toList(growable: false);
  ...
  RobotItem _toRobotItem(Map<String, dynamic> raw) {
    final capsRaw = raw['capabilities'];
    final capabilities = <String>[];
    if (capsRaw is List) {
      for (final c in capsRaw) {
        final value = c.toString().trim();
        if (value.isNotEmpty) capabilities.add(value);
      }
    }

    return RobotItem(
      id: (raw['id'] ?? '').toString(),
      name: (raw['name'] ?? 'Unnamed Robot').toString(),
      type: (raw['type'] ?? 'General').toString(),
      description: (raw['description'] ?? 'No description available').toString(),
      image: (raw['image'] ?? '').toString(),
...
class _RobotsCatalogPageState extends State<RobotsCatalogPage> {
  ...
  Future<void> _fetchRobots() async {
    ...
    try {
      final rows = await BackendApi.instance.fetchRobots();
      final mapped = rows.map(_toCatalogRobot).toList(growable: false);
  ...
  CatalogRobot _toCatalogRobot(Map<String, dynamic> raw) {
    final capsRaw = raw['capabilities'];
    final capabilities = <String>[];
    if (capsRaw is List) {
      for (final c in capsRaw) {
          ...
          children: [
            ...
            if (loading)
                        ...
                        Positioned(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                currentRobot.name,
                                style: TextStyle(fontSize: isMobile ? 34 : 56, fontWeight: FontWeight.w800),
                              ),
                              const SizedBox(height: 8),
                              Text(
                                ...
                                children: [
                                  ...
                                  FilledButton.icon(
                                    onPressed: () => Navigator.pushNamed(context, '/order', arguments: {
                                      ...
                                      'type': currentRobot.type,
                                      'description': currentRobot.description,
                                      'image': currentRobot.image,
                                      'price': currentRobot.price,
                                    }),
                                    icon: const Icon(Icons.chevron_right),
              ...
              const Padding(
                ...
                child: Text('No robots available right now.'),
              ),
          ],
        ),
      ),
...
class _ContactPageState extends State<ContactPage> {
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      await BackendApi.instance.submitContact(
        firstName: firstName,
        lastName: lastName,
        email: email,
        helpType: helpType,
...
class _SolarPanelsPageState extends State<SolarPanelsPage> {
  ...
  Future<void> _fetchSolarPanels() async {
    ...
    try {
      final rows = await BackendApi.instance.fetchSolarPanels();
      final mapped = rows.map(_toSolarPanelItem).toList(growable: false);
  ...
  SolarPanelItem _toSolarPanelItem(Map<String, dynamic> raw) {
    return SolarPanelItem(
      id: (raw['id'] ?? '').toString(),
      name: (raw['name'] ?? 'Unnamed Panel').toString(),
      type: (raw['type'] ?? 'General').toString(),
      description: (raw['description'] ?? 'High-efficiency solar panel technology').toString(),
      image: (raw['image'] ?? '').toString(),
      capacity: (raw['power'] ?? raw['capacity'] ?? 'N/A').toString(),
      efficiency: (raw['efficiency'] ?? 'N/A').toString(),
      price: (raw['price'] ?? '\$0').toString(),
      rating: double.tryParse((raw['rating'] ?? 0).toString()) ?? 0,
      reviews: int.tryParse((raw['reviews'] ?? 0).toString()) ?? 0,
    );
          ...
          '25-year performance warranty',
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(child: Text(s, style: const TextStyle(color: Color(0xFF334155), fontSize: 18))),
                ],
              ),
            )),
        const SizedBox(height: 10),
    ...
    return Container(
      ...
      child: const Column(
        children: [
          ...
          SizedBox(height: 8),
          Text('© 2026 Allytic Labs. All rights reserved.'),
        ],
      ),
    );
  }
...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _submitPassword() async {
    ...
    try {
      await BackendApi.instance.login(username: email, password: password);
